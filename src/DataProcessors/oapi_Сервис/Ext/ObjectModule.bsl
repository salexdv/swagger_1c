...
Процедура ПроверитьПараметрыЗагрузкиСервисовИзКонфигурации(ИсключаемыеСервисы = Неопределено,
	ИсключаемыеШаблоны = Неопределено,
	ИсключаемыеМетоды = Неопределено)
	
	Если ИсключаемыеСервисы <> Неопределено И ТипЗнч(ИсключаемыеСервисы) <> Тип("Массив") Тогда
		ТекстОшибки = НСтр("ru='Исключаемые сервисы должны передаваться массивом.'");
		ВызватьИсключение(ТекстОшибки);
	КонецЕсли;
	
	Если ИсключаемыеШаблоны <> Неопределено И ТипЗнч(ИсключаемыеШаблоны) <> Тип("Массив") Тогда
		ТекстОшибки = НСтр("ru='Исключаемые шаблоны должны передаваться массивом.'");
		ВызватьИсключение(ТекстОшибки);
	КонецЕсли;
	
	Если ИсключаемыеМетоды <> Неопределено И ТипЗнч(ИсключаемыеМетоды) <> Тип("Массив") Тогда
		ТекстОшибки = НСтр("ru='Исключаемые метода должны передаваться массивом.'");
		ВызватьИсключение(ТекстОшибки);
	КонецЕсли;
...
//
Функция ЗагрузитьСервисыИзКонфигурации(ИсключаемыеСервисы = Неопределено,
	ИсключаемыеШаблоны = Неопределено,
	ИсключаемыеМетоды = Неопределено) Экспорт
	
	ПроверитьПараметрыЗагрузкиСервисовИзКонфигурации(ИсключаемыеСервисы,
		ИсключаемыеШаблоны,
		ИсключаемыеМетоды);
	...
	Для Каждого Сервис Из Метаданные.HTTPСервисы Цикл
		...
		Если ИсключаемыеСервисы <> Неопределено И ИсключаемыеСервисы.Найти(Сервис.Имя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ШаблонСервиса Из Сервис.ШаблоныURL Цикл
			...
			Если ИсключаемыеШаблоны <> Неопределено И ИсключаемыеШаблоны.Найти(КлючШаблона) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Каждого МетодШаблона Из ШаблонСервиса.Методы Цикл
				...
				Если ИсключаемыеМетоды <> Неопределено И ИсключаемыеМетоды.Найти(КлючМетода) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
