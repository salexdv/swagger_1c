...
Функция СоздаватьТегиПриЗагрузке(ПараметрыЗагрузки)
	
	Возврат ПараметрыЗагрузки = Неопределено Или ПараметрыЗагрузки.СоздаватьТеги;
	
КонецФункции

Функция ПолучитьПараметрыИзАдресаШаблона(Адрес)
	
	Параметры = Новый Массив();
	
	НачалоПараметра = СтрНайти(Адрес, "{");
	
	Пока 0 < НачалоПараметра Цикл
		
		КонецПараметра = СтрНайти(Адрес, "}", , НачалоПараметра + 1);
		
		Если 0 < КонецПараметра Тогда
			
			ИмяПараметра = Сред(Адрес, НачалоПараметра + 1, КонецПараметра - НачалоПараметра - 1);
			
			Если ЗначениеЗаполнено(ИмяПараметра) Тогда
				Параметры.Добавить(ИмяПараметра);
			КонецЕсли;
			
			НачалоПараметра = СтрНайти(Адрес, "{", , КонецПараметра);
...
Процедура ДобавитьПараметрыШаблонаВМетод(Метод, Параметры)
	
	Для Каждого Параметр Из Параметры Цикл
		СтрокаПараметра = Метод.Сервис.ТипСтрока("", Параметр);
		Метод.ДобавитьПараметрЗапроса(Параметр, СтрокаПараметра, Истина);
	КонецЦикла;
...
//   Имя сервиса и имя шаблона следует разделять точкой. Например, "ПередачаДанных.ТомИПутьКФайлу"
...
//   Имя сервиса, имя шаблона и имя метода следует разделять точкой.
...
Функция ПараметрыЗагрузкиСервисовИзКонфигурации() Экспорт
	...
	Параметры.Вставить("СоздаватьТеги"   , Истина);
...
Функция ЗагрузитьСервисыИзКонфигурации(ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Для Каждого Сервис Из Метаданные.HTTPСервисы Цикл
		...
		Если СоздаватьТегиПриЗагрузке(ПараметрыЗагрузки) Тогда
			ИмяТега = Сервис.Имя;
		...
		Для Каждого ШаблонСервиса Из Сервис.ШаблоныURL Цикл
			...
			Параметры = ПолучитьПараметрыИзАдресаШаблона(АдресШаблона);
			...
			Для Каждого МетодШаблона Из ШаблонСервиса.Методы Цикл
				...
				ДобавитьПараметрыШаблонаВМетод(Метод, Параметры);
